...
### Fixed
- **Soil moisture source moved off dead NSM endpoint** — `nationalsoilmoisture.com` is no longer resolvable, so the integration now pulls volumetric soil moisture from the nearest USDA SCAN station (`SMS:*`) instead. It prefers the depth closest to 5 cm (about 2 in) and uses the newest available daily value.
...
- **SCAN depth parsing corrected** — SCAN depths are reported as negative inches below ground. Depth selection now compares by absolute depth, so 2-inch moisture and temperature readings are actually preferred, and the parser now uses the newest available daily value rather than the oldest value in the requested window.
...
### Added
- **Persistent notifications for upstream outages** — when the configured weather entity, weather forecast service, USDA SCAN station lookup, USDA SCAN soil-moisture fetch, or USDA SCAN soil-temperature fetch fails, the integration now creates a Home Assistant persistent notification on the first failure and dismisses it automatically once that source recovers.
